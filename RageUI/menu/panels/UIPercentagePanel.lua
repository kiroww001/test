local Percentage = {
    Background = { Dictionary = "tarekmenu", Texture = "gradient_bgd", Y = 4, Width = 431, Height = 76 },
...
function RageUI.PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(CurrentMenu.X + Percentage.Bar.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + Percentage.Bar.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Percentage.Bar.Width, Percentage.Bar.Height, 18, 22, 24, 200)
