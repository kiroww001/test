...
local SettingsButton = {
    ...
    SelectedSprite = { Dictionary = "tarekmenu", Texture = "gradient_nav", Y = 0, Width = 431, Height = 38 },
}
...
local SettingsSlider = {
    ...
    LeftArrow = { Dictionary = "tarekmenu", Texture = "arrowleft", X = 235, Y = 11.5, Width = 15, Height = 15 },
    RightArrow = { Dictionary = "tarekmenu", Texture = "arrowright", X = 400, Y = 11.5, Width = 15, Height = 15 },
}
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            ...
            if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
                ...
                if Selected then
                    RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 30, SettingsButton.SelectedSprite.Height - 3, 0, 115, 240, 255)
                ...
                RenderRectangle(
                    CurrentMenu.X + SettingsSlider.Background.X + (((SettingsSlider.Background.Width - SettingsSlider.Slider.Width) / (ProgressMax - 1)) * (ProgressStart - 1)) + CurrentMenu.WidthOffset - RightOffset,
                    CurrentMenu.Y + SettingsSlider.Slider.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset,
                    SettingsSlider.Slider.Width,
                    SettingsSlider.Slider.Height,
                    57, 116, 200, 255
                )
                
                if Divider then
                    local dividerX = SettingsSlider.Background.X 
                    + (SettingsSlider.Background.Width / 2) 
                    - (SettingsSlider.Divider.Width / 2) 
                    - RightOffset
                    
                    local dividerY = SettingsSlider.Background.Y 
                    + (SettingsSlider.Background.Height / 2) 
                    - (SettingsSlider.Divider.Height / 2)
                    
                    RenderRectangle(
                        CurrentMenu.X + dividerX + CurrentMenu.WidthOffset,
                        CurrentMenu.Y + dividerY + CurrentMenu.SubtitleHeight + RageUI.ItemOffset,
                        SettingsSlider.Divider.Width,
                        SettingsSlider.Divider.Height,
                        245, 245, 245, 255
                    )
                end
